...
export const BoundedContext = (props) => {
    if (props.currentContext && props.currentContext.id === props.context.id) {
        return (
            <span className="currentContext" onClick={toggleIsCurrentContext(props)} style={{ backgroundColor: props.context.color, padding: '5px' }}>
                {props.context.name}
            </span>
        )
    }
    return (
        <span className="context" onClick={toggleIsCurrentContext(props)} style={{ backgroundColor: props.context.color, padding: '5px' }}>
            {props.context.name}
        </span>
    );
};
